...
gulp.task('copy', function () {
  ...
  var elements_js = gulp.src(['app/elements/**/*.js'])
    .pipe(uglify())
    .pipe(gulp.dest('dist/elements'));
  ...
  var module = gulp.src(['app/modules/**/*.js'])
    .pipe(uglify())
...
gulp.task('cache-config', function (callback) {
  ...
  glob('{elements,modules,scripts,styles}/**/*.*', {cwd: dir}, function(error, files) {
    if (error) {
      callback(error);
    } else {
...
gulp.task('serve', ['styles', 'elements', 'images', 'modules'], function () {
  browserSync({
    port: 5000,
    notify: false,
    logPrefix: 'PSK',
  ...
  gulp.watch(['app/modules/**/*.scss'], ['modules', reload]);
  gulp.watch(['app/modules/**/*'], ['jshint', reload]);
  gulp.watch(['app/elements/**/*'], ['jshint', reload]);
  gulp.watch(['app/scripts/**/*', 'app/scripts/*'], ['jshint', reload]);    
